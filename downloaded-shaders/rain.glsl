...
{
    number resolution = 10. * exp2(-3.*iMouse.x/iResolution.x);
    ...
    for (int j = -MAX_RADIUS; j <= MAX_RADIUS; ++j)
    {
        for (int i = -MAX_RADIUS; i <= MAX_RADIUS; ++i)
        {
            ...
            number t = fract(0.3*iTime + hash12(hsh));
            vec2 v = p - uv;
            number d = length(v) - (number(MAX_RADIUS) + 1.)*t;

            number h = 1e-3;
            number d1 = d - h;
            number d2 = d + h;
            number p1 = sin(31.*d1) * smoothstep(-0.6, -0.3, d1) * smoothstep(0., -0.3, d1);
            number p2 = sin(31.*d2) * smoothstep(-0.6, -0.3, d2) * smoothstep(0., -0.3, d2);
            circles += 0.5 * normalize(v) * ((p2 - p1) / (2. * h) * (1. - t) * (1. - t));
        }
    }
    circles /= number((MAX_RADIUS*2+1)*(MAX_RADIUS*2+1));

    number intensity = mix(0.01, 0.15, smoothstep(0.1, 0.6, abs(fract(0.05*iTime + 0.5)*2.-1.)));
